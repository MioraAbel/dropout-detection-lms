...
def faire_prediction():
    ...
    # 1. On lance la prédiction
    resultats = predire(data)

    connection = get_connection()
    cursor = connection.cursor()

    # 2. Mise à jour de l'étudiant (Bug corrigé ici : on met bien le risque en texte)
    cursor.execute("""
        UPDATE student 
        SET niveau_risque              = %s,
        ...
        WHERE std_id = %s
    """, (
        resultats["risque_decrochage"],       # "élevé", "modéré" ou "faible"
        resultats["probabilite_decrochage"],  # ex: 0.85
        resultats["probabilite_non_decrochage"], # ex: 0.15
        resultats["status"],                  # "risque" ou autre
        data.get("userid")
    ))

    # 3. Sauvegarde dans l'historique si l'étudiant est à risque élevé
